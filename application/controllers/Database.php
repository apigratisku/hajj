...
class Database extends CI_Controller
{
    ...
    {
        $this->load->model('transaksi_model');
        $data['title'] = 'Filter Done';

        $data['email_domain_list'] = $this->transaksi_model->get_unique_email_domains_done();
        $allowed_domains = array_map(function ($row) {
            return $row->email_domain;
        }, $data['email_domain_list']);
        ...
        $base_url = base_url('database/filter_done');
        $query_params = [];
        ...
        $this->load->view('templates/sidebar');
        $this->load->view('templates/header', $data);
        $this->load->view('database/filter_done', $data);
        $this->load->view('templates/footer');
    }
    ...
    {
        $this->load->model('transaksi_model');
        $data['title'] = 'Filter Data > 1 Tahun';

        $data['email_domain_list'] = $this->transaksi_model->get_unique_email_domains_done_1tahun();
        $allowed_domains = array_map(function ($row) {
            return $row->email_domain;
        }, $data['email_domain_list']);
        ...
        $data['peserta'] = $this->transaksi_model->get_paginated_filtered_done_1tahun($per_page, $offset, $filters);
        $data['flag_doc_list'] = $this->transaksi_model->get_unique_flag_doc_done_1tahun();

        $total_rows = $this->transaksi_model->count_filtered_done_1tahun($filters);

        $this->load->library('pagination');

        $base_url = base_url('database/filter_done_1_tahun');
        $query_params = [];
        ...
        $this->load->view('templates/sidebar');
        $this->load->view('templates/header', $data);
        $this->load->view('database/filter_done_1_tahun', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function process_import()
    {
        ...
        $mailbox = trim($this->input->post('mailbox') ?: ('mailbox@' . $email_prefix));
    ...
    public function download_successful_data_cpanel_forwarding()
    {
        ...
        try {
            ...
            // Add data
            // === Ambil konfigurasi email dari session (diset saat process_import) ===
