...
class Cpanel {
    ...
    public function __construct($config = array())
    {
        ...
        $this->cpanel_port = isset($config['port']) ? $config['port'] : 2083; // SSL biasanya 2083
    }

    /**
     * Login dan ambil session token cpsessxxxx
     */
    private function login()
    {
        ...
        // Log untuk debugging
        log_message('info', 'CPanel Login - HTTP Code: ' . $http_code);
        log_message('info', 'CPanel Login - Response Body: ' . $body);
        log_message('info', 'CPanel Login - Headers: ' . $header);

        if ($error) {
            log_message('error', 'CPanel Login - cURL Error: ' . $error);
            return false;
        }
        ...
        if (isset($json['status']) && $json['status'] == 1) {
            // simpan token & cookie
            $this->session_token = $json['security_token']; // contoh: "/cpsess1234567890"
            log_message('info', 'CPanel Login - Session Token: ' . $this->session_token);

            // ambil cookie login (kalau dibutuhkan)
            if (preg_match_all('/Set-Cookie:\s*([^;]*)/mi', $header, $matches)) {
                $this->cookies = implode("; ", $matches[1]);
                log_message('info', 'CPanel Login - Cookies: ' . $this->cookies);
            }

            return true;
        }

        log_message('error', 'CPanel Login - Failed: ' . $body);
        return false;
    }

    /**
     * Request API dengan session token
     */
    private function request($url)
    {
        ...
        log_message('info', 'CPanel Request - Endpoint: ' . $endpoint);
        ...
        log_message('info', 'CPanel Request - HTTP Code: ' . $http_code);
        log_message('info', 'CPanel Request - Response: ' . $result);

        if ($error) {
            log_message('error', 'CPanel Request - cURL Error: ' . $error);
            return ["error" => "Request error: " . $error];
        }

        if ($http_code !== 200) {
            log_message('error', 'CPanel Request - HTTP Error: ' . $http_code);
