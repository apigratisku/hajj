            ...
            <div class="card mobile-card">
                <div class="card-header bg-brown text-white d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <div class="d-flex align-items-center">
                        <h5 class="mb-0 me-3">Arsip Data Peserta</h5>
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        ...
                        <button type="button" class="btn btn-sm btn-success" onclick="restoreSelectedData()" id="restoreSelectedBtn" style="display: none;">
                            <i class="fas fa-undo"></i> <span class="d-none d-sm-inline">Kembalikan Terpilih</span>
                        </button>
                ...
                <div class="card-body p-0">
                    ...
                    <div class="mobile-search-container d-block d-md-none">
                        <div class="search-toggle" onclick="toggleMobileSearch()">
                            <i class="fas fa-search"></i> Cari Data Arsip
                        </div>
                        <div class="mobile-search-form" id="mobileSearchForm" style="display: none;">
                            <form method="get" action="<?= base_url('database/arsip') ?>" class="mobile-form" id="mobileSearchForm">
                                ...
                                <div class="form-actions">
                                    ...
                                    <a href="<?= base_url('database/arsip') ?>" class="btn btn-reset">
                    ...
                    <div class="desktop-search-container d-none d-md-block">
                        <form method="get" action="<?= base_url('database/arsip') ?>" class="desktop-form">
                            <div class="row g-2 align-items-center">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <a href="<?= base_url('database/arsip') ?>" class="btn btn-brown-light btn-sm">
                                        <i class="fas fa-times"></i> Reset
                                    </a>
                                </div>
                            </div>
                        ...
                        <div class="col-12">
                            <div class="alert alert-info">
                                <h6 class="alert-heading">
                                    <i class="fas fa-chart-bar"></i> Statistik Update Data Arsip
                                </h6>
                                <p class="mb-2">
                                    ...
                                    <tbody id="mobile-table-tbody">
                                        ...
                                        <tr data-id="<?= $p->id ?>">
                                            <td class="col-checkbox">
                                                <input type="checkbox" class="data-checkbox" value="<?= $p->id ?>" onchange="updateRestoreButton()">
                                            </td>
...
#selectAllMobile, #selectAllDesktop {
    width: 20px;
    height: 20px;
    cursor: pointer;
    accent-color: var(--success-color);
...
.mobile-excel-table .col-checkbox {
    min-width: 30px;
    max-width: 40px;
}
...
// Checkbox functionality for mass restore
function initializeCheckboxFunctionality() {
    updateRestoreButton();
}

function toggleSelectAllMobile(checkbox) {
    const dataCheckboxes = document.querySelectorAll('.mobile-excel-table .data-checkbox');
...
function toggleSelectAllDesktop(checkbox) {
    const dataCheckboxes = document.querySelectorAll('#transaksi-table .data-checkbox');
    dataCheckboxes.forEach(cb => {
        cb.checked = checkbox.checked;
    });
    updateRestoreButton();
...
function updateRestoreButton() {
    ...
    if (checkedBoxes.length > 0) {
        ...
        restoreBtn.innerHTML = `<i class="fas fa-undo"></i> <span class="d-none d-sm-inline">Kembalikan (${checkedBoxes.length})</span>`;
    } else {
...
function restoreSelectedData() {
    ...
    if (checkedBoxes.length === 0) {
        showAlert('Silakan pilih data yang akan dikembalikan dari arsip', 'warning');
        return;
    }
    
    const selectedIds = Array.from(checkedBoxes).map(cb => cb.value);
    
    if (confirm(`Apakah Anda yakin ingin mengembalikan ${selectedIds.length} data dari arsip?`)) {
        // Show loading state
        const restoreBtn = document.getElementById('restoreSelectedBtn');
        const originalText = restoreBtn.innerHTML;
        restoreBtn.disabled = true;
        restoreBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> <span class="d-none d-sm-inline">Memproses...</span>';
        
        // Send request to restore multiple data
        fetch('<?= base_url('database/restore_multiple_from_arsip') ?>', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-Requested-With': 'XMLHttpRequest'
        ...
        .then(result => {
            ...
            if (result.success) {
                showAlert(`Berhasil mengembalikan ${result.restored_count} data dari arsip`, 'success');
                
                // Redirect back to arsip page with current filters
                setTimeout(() => {
                    const currentUrl = new URL(window.location.href);
                    const params = new URLSearchParams(currentUrl.search);
                    
                    // Build redirect URL with current filters
                    let redirectUrl = '<?= base_url('database/arsip') ?>';
                    const queryParams = [];
                ...
                showAlert('Gagal mengembalikan data: ' + result.message, 'error');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            showAlert('Terjadi kesalahan saat mengembalikan data dari arsip', 'error');
        })
        .finally(() => {
            // Reset button state
