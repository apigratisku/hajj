            ...
            <div class="card mobile-card">
                <div class="card-header bg-brown text-white d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <div class="d-flex align-items-center flex-wrap gap-2">
                        <h5 class="mb-0 me-3">Filter Data > 1 Tahun</h5>
                        <span class="badge bg-light text-dark">
                            <i class="fas fa-info-circle"></i> Arsip Done, penjadwalan &gt; 1 tahun
                        </span>
                    </div>
                </div>
                <div class="card-body p-0">
                    ...
                    <div class="mobile-search-container d-block d-md-none">
                        ...
                        <div class="mobile-search-form" id="mobileSearchForm" style="display: none;">
                            <form method="get" action="<?= base_url('database/filter_done_1_tahun') ?>" class="mobile-form">
                                ...
                                <div class="form-actions">
                                    ...
                                    <a href="<?= base_url('database/filter_done_1_tahun') ?>" class="btn btn-reset">
                    ...
                    <div class="desktop-search-container d-none d-md-block p-3">
                        <form method="get" action="<?= base_url('database/filter_done_1_tahun') ?>" class="desktop-form">
                            <div class="row g-2 align-items-end">
                                ...
                                <div class="col-md-auto">
                                    ...
                                    <a href="<?= base_url('database/filter_done_1_tahun') ?>" class="btn btn-brown-light btn-sm">
                                        <i class="fas fa-times"></i> Reset
                                    </a>
                                </div>
                            </div>
                    ...
                    <div class="table-responsive d-none d-lg-block">
                            <table class="table table-bordered table-striped table-hover" id="transaksi-table">
                                ...
                                <?php if (empty($peserta)): ?>
                                    <tr>
                                        <td colspan="<?= $this->session->userdata('role') == 'admin' ? '14' : '12' ?>" class="text-center">Tidak ada data</td>
                                    </tr>
                                <?php else: ?>
...
<!-- Operator Statistics Modal (disabled on Filter Data > 1 Tahun page) -->
...
<div class="modal fade" id="registerUlangStatisticsModal" tabindex="-1" aria-labelledby="registerUlangStatisticsModalLabel" aria-hidden="true">
    ...
    <script>
const FILTER_DONE_1_TAHUN_BASE_URL = '<?= base_url('database/filter_done_1_tahun') ?>';

function buildFilterDone1TahunRedirectUrl() {
    const params = new URLSearchParams(window.location.search);
    let redirectUrl = FILTER_DONE_1_TAHUN_BASE_URL;
    const queryParams = [];
...
function saveRow(button) {
    ...
    .then(result => {
        ...
        if (result.success) {
            ...
            setTimeout(() => {
                window.location.href = buildFilterDone1TahunRedirectUrl();
            }, 1000);
...
function deleteData(id) {
    if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
        const redirectUrl = buildFilterDone1TahunRedirectUrl();
...
document.addEventListener('DOMContentLoaded', function() {
    updateDeleteButton();
    initializeFlagDocSearch();
});

// Searchable Select Functions
function initializeFlagDocSearch() {
    const desktopSelect = document.getElementById('flag_doc_desktop');
    const mobileSelect = document.getElementById('flag_doc_mobile');
    
    if (desktopSelect) {
        setupSearchableSelect(desktopSelect);
    }
    
    if (mobileSelect) {
        setupSearchableSelect(mobileSelect);
    }
}
...
function setupSearchableSelect(selectElement) {
    ...
    input.addEventListener('focus', function() {
        populateDropdown(this.value.trim());
        dropdown.style.display = 'block';
        currentHighlight = -1;
    });
